...
def plot_composite_analysis(plotName, epsilon, N_0_squared, simulations, window_size=5000):
    ...
    for i in range(5):
        axs[3, 0].plot(time_array, abs(eta_list[i]), linewidth=0.5, linestyle='--', color='gray')
    axs[3, 0].plot(time_array, average_eta, label='Average', linewidth=1.5, color='blue')
    axs[3, 0].set_title(f'Average eta')
    axs[3, 0].grid()

    for i in range(5):
        axs[3, 1].plot(time_array, phi_e_list[i][:, 1], linewidth=0.5, linestyle='--', color='gray')
    axs[3, 1].plot(time_array, average_phi_e[:, 1], label='Average', linewidth=1.5, color='blue')
    axs[3, 1].set_title(f'Average b_e')
    axs[3, 1].grid()
