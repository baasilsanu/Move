...
#Make sure that all the markers are removed.
...
class Simulation:
    def __init__(self, epsilon, N_0_squared, r_m, k, m, m_u, dt, total_time):
        ...
        self.time_values = np.arange(0, self.total_time, .001)
...
if __name__ == "__main__":
    ...
    #Build the rest of the for loop where it loops through the n0 and eps values.

    eps_vals = [0.07894281881282045, 0.14509802140045203, 0.24142071558123343, 0.3055431793036454]
    N_0_squared_vals = [149.82931246564826, 415.1406776514857, 973.6529697935448, 1444.3802061982885]
    ...
    def run_simulation(params):
        i, epsilon, N_0_squared, r_m, k, m, m_u, dt, total_time = params
        print(f'Running iteration {i} with epsilon={epsilon} and N_0_squared={N_0_squared}')
        sim = Simulation(epsilon, N_0_squared, r_m, k, m, m_u, dt, total_time)
        sim.simulate()
        rev, dur = getTimesAndDurations(sim.U_history, sim.time_values, sim.total_time)
        result = (sim.epsilon, sim.N_0_squared, sim.U_history, sim.R_vals, rev, dur)
        del sim
        return result
    ...
    multi_ind = 1
    for i in eps_vals:
        for j in N_0_squared_vals:
            for item in range(1):                                              #Change this for full run
                params.append((multi_ind, i, j, r_m, k, m, m_u, dt, total_time))
                multi_ind += 1
    # Use multiprocessing to run simulations in parallel
    with multiprocessing.Pool() as pool:
        results = pool.map(run_simulation, params)

    for eps, N_0_squared_temp, U_history, R_vals, rev, dur in results:
        eps_list.append(eps)
        N_0_squared_list.append(N_0_squared_temp)
        U_list.append(U_history)
        R_list.append(R_vals)
        reversals.append(np.array(rev))
        durations.append(np.array(dur))     
    ...
    #     sim.simulate()
    #     rev, dur = getTimesAndDurations(sim.U_history, sim.time_values, sim.total_time)
    #     U_list.append(sim.U_history)
    #     R_list.append(sim.R_vals)
    #     reversals.append(np.array(rev))
    #     durations.append(np.array(dur))
    #     del sim
    ...
    for i in range(len(U_list)):
        ...
        reversals_json = json.dumps(reversals[i].tolist())
        durations_json = json.dumps(durations[i].tolist())

    ##The variables in the for loop have weird names. They are not lists.

        cur.execute("""
        INSERT INTO grid_data (eps, n_0_squared, U_list, R_list, Reversals_list, Durations_list) 
        VALUES (%s, %s, %s, %s, %s, %s)
        """, (eps_list[i], N_0_squared_list[i], U_list_json, R_list_json, reversals_json, durations_json))
